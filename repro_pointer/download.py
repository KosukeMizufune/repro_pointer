...
def urlretrieve(url, zip_filename):
    if isinstance(zip_filename, str):
        zip_filename = Path(zip_filename)
    if not zip_filename.parent.exists():
        zip_filename.parent.mkdir(parents=True)
    request.urlretrieve(url, zip_filename, _reporthook)
